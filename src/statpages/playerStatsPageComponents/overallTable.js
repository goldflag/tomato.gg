...
import { MoEStars, Pagination, Banner } from "Components";
import {
  ClassFilter,
  GlobalFilter,
  MasteryFilter,
...
function OverallTable({ data, setSelectedTank, setModalOpen }) {
  ...
  return (
    <>
      <FiltersContainer>
        <Banner style={{marginBottom: "1rem"}} color={"linear-gradient(90deg, rgba(72,152,47,1) 0%, rgba(37,199,168,1) 53%, rgba(0,255,245,1) 100%)"}>NEW FEATURE: CLICK ON ROW TO SEE DETAILED TANK STATS!</Banner>
        ...
        {headerGroups.map((headerGroup, i) => (
          <React.Fragment key={i}>
            <ButtonFiltersContainer key={i}>
              {filterOrder.map(
                (n) => !columns[n].disableFilters && <span key={n}>{headerGroup.headers[n].render("Filter")}</span>
              )}
            </ButtonFiltersContainer>

    ...
    function Table({ columns, data, updateMyData, skipReset }) {
        ...
        function SubRows({ data }) {
            if (data) {
                console.log(data);
                return (
                    <div style={{ backgroundColor: theme === 'dark' ? 'rgb(40, 40, 40)' : 'rgb(255, 255, 255)', marginLeft: '-0.5rem' }}>
                        <MoEGraph data={data} />
                    </div>
                );
            }
    ...
    */}
    ...
    const columns = React.useMemo(
        () => [
            ...
            {
                Cell: ({ value }) => { return (<img src={require(`../../assets/classIcons/${value}.png`)} style={{ maxWidth: '20px', margin: '3px' }} alt={value} />) },
                Header: 'Class',
                accessor: 'class',
                Filter: ClassFilter,
                filter: 'equals',

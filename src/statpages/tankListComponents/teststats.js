...
export default function Table() {
    ...
    async function fetchData() {
        ...
        for (let i = 0; i < data.length; ++i) {
            const link = `/tank/${data[i].id}/${data[i].name.replace(
                /\s/g,
                "-"
            )}`;
            data[i].id = (
                <Link to={link}>
                    <img
                        src={require(`../../assets/tankIcons/${data[i].id}.png`)}
                        style={{
                            maxHeight: "24px",
                            margin: "-0.3rem",
                            paddingLeft: "0.7rem",
    ...
    const header = (
        <div className="table-header">
            ...
            <span className="p-input-icon-left">
                ...
                <InputText
                    type="search"
                    onInput={(e) => setGlobalFitler(e.target.value)}
        ...
        return (
            <div
                style={{
                    color: "white",
                    backgroundColor: WRc(
                        rowData.winrate.substring(0, rowData.winrate.length - 1)
                    ),
                    padding: "0.5rem 1rem",
    ...
    function WN8color(rowData) {
        return (
            <div
                style={{
                    color: "white",
                    backgroundColor: WN8c(rowData.wn8),
                    padding: "0.5rem 1rem",
                    margin: "-1rem -1rem -1rem -1rem",
                }}
            >
                ...
                <div className="datatable">
                    <div className="card">
                        <DataTable
                            ...
                            rows={100}
                            rowsPerPageOptions={[50, 100, 250, 1000]}
                            sortField="wn8"
                            sortOrder={-1}
                            header={header}
